...
class Numeric(nn.Module):
    ...
    def forward(self, X, state):
        ...
        X = X.reshape(-1, *X.T.shape)  # [direction, batch_size, seq_len]
    ...
    def predict(self, prefix, num_preds, device):  #@save
        ...
        outputs = torch.zeros(num_preds, device=device)
        for y in prefix:  # Warm-up period
            y_hat, state = self.forward(y.reshape(1,1), state)

        # outputs[0] = self.forward(prefix[-1].reshape(1,1), state)
        outputs[0] = y_hat
        for i in range(num_preds):  # Predict `num_preds` steps
            y, state = self.forward(outputs[i].reshape(1,1), state)
            outputs[i] = y.reshape(-1)
        return outputs
